...
def resnet_model():
    pretrained_model=ResNet50(weights='imagenet',include_top=False,input_shape=data.shape[1:])
    ...
    model.fit(train_data, train_target, epochs=20, verbose=1, validation_split=0.2)
    ...
    model.fit(train_data, train_target, epochs=10, verbose=1, callbacks=[checkpoint], validation_split=0.2)
